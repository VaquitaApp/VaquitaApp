...
| D3 | Confirmación de email en registro (HU05-2) | 🟢 Resuelto | Opción B: registro directo, token inmediato, sin verificación de email en E1. |
...
| Registro | HU05-2 (SCRUM-36) | ⚠️ Confirmación email — ver D3 |
| Invitar/eliminar participante | HU11 (SCRUM-23) | ⚠️ Sin criterios, ver D7 |
...
7. **HU05-2 (SCRUM-36): confirmación de email** — Para E1 local sin SMTP esto no funciona. Ver D3.

8. **HU15 (SCRUM-54): pago real al destinatario** — Demasiado complejo para MVP local. Ver D4.

9. **HU16 (SCRUM-55): notificaciones con email automático** — Requiere servicio externo. Ver D5.
...
| SCRUM-36 (HU05-2) | Simplificar registro según D3 |
| SCRUM-52 (HU13) | ✅ Criterios completos. Sin cambios. |
| SCRUM-53 (HU14) | ✅ Criterios completos. Confirmar en descripción que comparte componente con HU08. |
| SCRUM-54 (HU15) | Resolver D4 — probable mover a E2 |
| SCRUM-55 (HU16) | Mantener en E1. Actualizar criterios con lógica de mora definida + Ethereal Email. Definir umbral de días. |
...
| ID | Supuesto | Impacto |
|---|---|---|
| S-01 | El registro de usuario no requiere verificación de email en E1. El token se emite inmediatamente al completar el formulario. | Simplifica HU05-2. En E2 se puede agregar verificación opcional. |
...
| S-07 | Todo flujo de pago en E1 es simulado: la UI presenta formulario visual con campos de tarjeta (`<MockPaymentForm>`), el servidor registra la transacción como exitosa sin procesar dinero real. Datos persistidos: `{ transactionId: 'sim_...', amount, status: 'succeeded', provider: 'simulation' }`. La lógica se encapsula en `PaymentService` con firma idéntica a Stripe. En E2: reemplazar cuerpo de `processContribution()` + cambiar `<MockPaymentForm>` por `<CardElement>` de Stripe Elements. Cero cambio de esquema ni de rutas. | Afecta `server/services/paymentService.js` y el componente de pago en frontend. Ninguna dependencia externa de pago en E1. |
...
| 2026-04-27 | Restricciones de edición de fondo | Solo editable en estado `activo`. Con aportes registrados, `montoEsperado`, `fechaLimite` y `cuentaDestinatario` quedan bloqueados. Ver S-15. |
