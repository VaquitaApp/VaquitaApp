...
describe('POST /api/auth/register', () => {
  test('201 with message — no token returned', async () => {
    const res = await request(app)
      ...
      .send({ name: 'Ana', email: 'ana@test.com', password: 'pass1234', rut: '11.111.111-1' });
  ...
  test('409 if email already in use', async () => {
    ...
    const res = await request(app)
      .post('/api/auth/register')
      .send({ name: 'Otro', email: 'ana@test.com', password: 'pass1234', rut: '11.111.111-1' });
...
describe('GET /api/auth/verify-email/:token', () => {
  test('200 and sets isEmailVerified on valid token', async () => {
    const user = await createUser({ email: 'ana@test.com', isEmailVerified: false, emailVerificationToken: 'tok-123' });
  ...
  test('404 on unknown token', async () => {
    const res = await request(app).get('/api/auth/verify-email/does-not-exist');
...
describe('POST /api/auth/login', () => {
  ...
  test('403 when email is not verified', async () => {
    await createUser({ email: 'unverified@test.com', passwordHash: 'pass1234', isEmailVerified: false });
    const res = await request(app)
      .post('/api/auth/login')
...
describe('GET /api/auth/me', () => {
  test('200 returns authenticated user', async () => {
    const user = await createUser({ email: 'ana@test.com', passwordHash: 'pass1234' });
